...
func (data QRData) GenerateString() (string, error) {
	...
	for identifier, idValue := range data.MerchantIdentifiers {
		...
		for tag, value := range idValue {
			...
			customData += fmt.Sprintf("%02d", len([]rune(value)))
			customData += value
		}
		if len([]rune(customData)) > 99 {
			return "", errors.New("custom identifier too long (max length: 99 characters)")
		}
		customData = fmt.Sprintf("%02d", identifier) + fmt.Sprintf("%02d", len([]rune(customData))) + customData
		qr += customData
	}
	...
	qr += "59" + fmt.Sprintf("%02d", len([]rune(data.MerchantName))) + data.MerchantName
	qr += "60" + fmt.Sprintf("%02d", len([]rune(data.City))) + data.City
	qr += "6304"
